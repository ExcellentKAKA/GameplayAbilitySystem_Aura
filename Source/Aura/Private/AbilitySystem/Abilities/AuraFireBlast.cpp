...
FString UAuraFireBlast::GetDescription(int32 Level)
{
	...
	return FString::Printf(TEXT(
			...
			"<Default>Launches %d </>"
			"<Default>fire balls in all firections, each coming back and</>"
...
TArray<AAuraFireBall*> UAuraFireBlast::SpawnFireBalls()
{
	...
	for(const FRotator& Rotator : Rotators)
	{
		...
		AAuraFireBall* FireBall = GetWorld()->SpawnActorDeferred<AAuraFireBall>(FireBallClass, SpawnTransform,
			GetOwningActorFromActorInfo(), CurrentActorInfo->PlayerController->GetPawn(), ESpawnActorCollisionHandlingMethod::AlwaysSpawn);
